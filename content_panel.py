...
class ContentPanel(BasePanel):
    ...
    def draw_keyboard(self, start_x, start_y):
        ...
        for row in self.keys:
            ...
            for key_id, size in row:
                ...
                key_str = key_mappings[key_id] if key_id is not None else ''
    ...
    def draw(self):
        ...
        if self.list_panel.selected_option and self.list_panel.selected_option.decode(
                'utf-8') == "Keyboard":
            self.draw_keyboard(left_panel_width + 20,
                               20)  # Draw keyboard with some padding
