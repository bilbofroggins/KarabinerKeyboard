    ...
    def draw_help(self, start_x, start_y):
        if self.quitting_done_flag[0] == True:
            ...
            DrawText(b"Restarting...",
                     start_x, start_y, Config.font_size * 4, BLACK)
            return
        ...
        if not self.is_updated:
            DrawText(f"Update available: {self.latest_version}".encode('utf-8'),
                     start_x, start_y, Config.font_size, BLACK)
            start_y += Config.font_size * 2
            DrawingHelper.clickable_link("Update", start_x, start_y, Config.font_size, BLACK, self.update_app, [self.latest_version])
