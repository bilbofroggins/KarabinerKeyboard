...
class UpdateView():
    def __init__(self):
        super().__init__()
        self.text1 = b"New version available"
        self.text2 = "Download & Install"
        width = MeasureText(self.text1, config.font_size)
        self.is_updated = True
        self.text_width = max(width, MeasureText(self.text2.encode('utf-8'), config.font_size))
    ...
    def draw_updates(self, start_row, start_col, max_row, max_col):
        if self.quitting_done_flag[0] == True:
            ...
            DrawRectangle(start_col, start_row, max_col, max_row, config.background_color)
        ...
        if not self.is_updated:
            ...
            DrawRectangle(box_start_col, box_start_row, max_col - box_start_col, max_row - box_start_row, BLUE)
            DrawText(self.text1, box_start_col + config.generic_padding, box_start_row + config.generic_padding, config.font_size, config.default_text_color)
